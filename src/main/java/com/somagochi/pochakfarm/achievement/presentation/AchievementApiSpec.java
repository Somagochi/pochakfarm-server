...
public interface AchievementApiSpec {
  ...
  @Operation(
      summary = "업적 목록 조회",
      description =
          "로그인한 사용자의 업적 목록을 진행도와 함께 커서 기반으로 조회한다(노출 순서대로, 한 페이지 20개). "
              + "category 를 지정하면 해당 카테고리만, cursor 에 이전 응답의 nextCursor 를 넘기면 다음 페이지를 반환한다. "
              + "진행도는 저장값이 아니라 조회 시점에 레벨과 포획 기록에서 다시 계산하며, 목표에 도달했는데 달성 기록이 없으면 그 자리에서 달성으로 확정한다. "
              + "달성 판정은 현재 페이지가 아니라 전체 업적을 대상으로 하므로, 첫 페이지만 조회해도 뒤 페이지 업적의 달성이 누락되지 않는다. "
              + "current 는 목표를 넘어도 실제값을 그대로 반환하므로 진행률 표시에는 target 으로 잘라 쓴다. "
              + "achieved 가 true 인 항목만 achievedAt 이 채워지고, rewardClaimed 가 false 면 보상을 수령할 수 있다. "
  ...
  ApiResponse<CursorPage<AchievementResponse>> getAchievements(
      @Parameter(description = "카테고리 필터. 생략하면 전체", example = "LEVEL") AchievementCategory category,
