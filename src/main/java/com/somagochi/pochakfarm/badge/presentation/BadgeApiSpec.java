...
public interface BadgeApiSpec {
  @Operation(
      summary = "내 뱃지 보관함 조회",
      description =
          "로그인한 사용자가 획득한 뱃지를 뱃지 코드 오름차순, 커서 기반으로 조회한다(한 페이지 20개). "
              + "category 를 지정하면 해당 카테고리(ACHIEVEMENT: 업적 뱃지, GYM_LEADER: 관장 뱃지)만, 생략하면 전체를 반환한다. "
              + "cursor 에 이전 응답의 nextCursor(마지막 항목의 뱃지 코드)를 넘기면 다음 페이지를 반환한다. "
  ...
  @io.swagger.v3.oas.annotations.responses.ApiResponse(
      ...
      content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
  ApiResponse<OwnedBadgePage> getOwnedBadges(
      @Parameter(description = "카테고리 필터. 생략하면 전체", example = "GYM_LEADER") BadgeCategory category,
      @Parameter(description = "다음 페이지 커서(이전 응답의 nextCursor). 첫 페이지는 생략", example = "BDG005")
          String cursor,
