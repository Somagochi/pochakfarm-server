...
public interface CaptureApiSpec {
  ...
  @Operation(
      summary = "포착 동물 농장 저장",
      description = "서버에서 생성한 누끼 이미지를 사용해 Capture 타입 농장의 선택 슬롯에 동물을 저장하거나 명시한 기존 동물을 교체한다.")
  @SecurityRequirement(name = "bearerAuth")
  @io.swagger.v3.oas.annotations.responses.ApiResponse(
      responseCode = "200",
  ...
  @io.swagger.v3.oas.annotations.responses.ApiResponse(
      responseCode = "400",
      description = "잘못된 배치 요청(INVALID_PARAMETER)",
      content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
  @io.swagger.v3.oas.annotations.responses.ApiResponse(
      responseCode = "403",
  ...
  @io.swagger.v3.oas.annotations.responses.ApiResponse(
      responseCode = "404",
      description = "Capture 또는 농장 슬롯 없음(CAPTURE_NOT_FOUND, FARM_SLOT_NOT_FOUND)",
      content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
  @io.swagger.v3.oas.annotations.responses.ApiResponse(
      responseCode = "409",
